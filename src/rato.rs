...
/*
     pub fn new(event_handler: T) -> Rato<T> {
         Rato {
             event_handler
         }
     }
    ...
    pub fn make_pub_sub_cmd(channels: Vec<&str>) -> Vec<u8> {
     ...
     fn handle_command<T>(event_handler: &T, conn_tags: &mut HashMap<String,String>,
        ...
        close_session: &mut bool
         ...
         debug!("DB:{}, Command:{}, Number of args:{}", String::from_utf8_lossy(&db), String::from_utf8_lossy(&args[0]),args.len());
        ...
        }else if  RedisUtil::arg_match(&args[0], "HSET")
        {
            ...
            match args.len() {
                ...
                4 => {
                    if RedisUtil::arg_match(&args[1], "NODES") && RedisUtil::arg_match(&args[2], "REMOVE"){
                        if let Err(e) =   event_handler.on_cmd_cluster_remove_node(&args[3]) {

                            format!(
                                "-ERR CLUSTER NODES REMOVE Failed '{}'\r\n",
                                e.to_string()
                            ).into_bytes()
                                .to_vec()
                        }else {
                ...
                13  => {
                    ...
                    for i in (3..args.len()).step_by(2) {
                        if i + 1 < args.len() {
                            debug!("CLUSTER NODES ADD/UPDATE: Key: {}, Val:{}",
                                   String::from_utf8_lossy(&args[i]),
                                   String::from_utf8_lossy(&args[i + 1]));
                            kv.insert(&args[i], &args[i + 1]);
                        } else {
                    ...
                    if RedisUtil::arg_match(&args[2], "ADD") {
                        ...
                        if let Err(e) = event_handler.on_cmd_cluster_update_node(&kv) {
                            format!(
                                "-ERR CLUSTER NODES UPDATE Failed '{}'\r\n",
                                e.to_string()
                            ).into_bytes()
                                .to_vec()
                        } else {
